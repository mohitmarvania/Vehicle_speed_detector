...
# Create a folder to track total car exceeded speed limit and other information.
...
class DistTracker:
    def __init__(self):
        ...
        # Creating an id for number of vehicles.
        self.id_count = 0
        # self.start = 0
        # self.stop = 0
        self.et = 0
    ...
    '''
    self.s1, self.s2, and self.s are NumPy arrays of shape (1, 1000).
    These arrays are likely used to store time-related data for individual tracked objects.
    s1 -> stores start time.
    s2 -> stores stop time.
    s -> stores difference between start and stop time.
    
    self.f and self.capf are NumPy arrays of shape (1000).
    These arrays are used to keep track of flags and states for individual tracked objects.
    f -> flag set to 1 if an object is in certain region and set to 0 if not.
    capf -> flag to indicate whether an object's information has been captured.
    '''
